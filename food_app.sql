...
CREATE TABLE foods (
    ...
    FOREIGN KEY(type_id) REFERENCES food_types(type_id) ON DELETE CASCADE -- delete all the child rows when delete this type
);
...
('Side Salad', 2.80, 3),
('Garlic Bread', 1.50, 4),
('Chicken Wings', 5.50, 5),
('Mozzarella Sticks', 4.50, 2),
('Coleslaw', 1.80, 3),
('Mac and Cheese', 3.70, 5),
('Grilled Vegetables', 4.20, 4);

-- Bài tập query
-- 1. Tìm 5 người đã like nhà hàng nhiều nhất.
SELECT full_name, email, COUNT(date_like) AS total_likes
FROM users
...
SELECT res_name, COUNT(date_like) AS total_likes
FROM restaurants
...
SELECT full_name, email, COUNT(*) AS total_orders
FROM users
INNER JOIN orders ON users.user_Id = orders.user_id
GROUP BY full_name, emails
...
WHERE order_id IS NULL AND rate_res_id IS NULL AND like_res_id IS NULL;